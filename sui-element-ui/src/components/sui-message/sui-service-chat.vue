...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getUserList() {
        const that = this;
        that.$request.doPost('/admin/sysMessage/getClientUserList').then(res => {
          if (res.isOk) {
      ...
      getChatList(){
        const that = this;
        that.$request.doPost('/admin/sysMessage/getChatList',{refId:that.refId}).then(res => {
          if (res.isOk) {
            that.chatList = [];
      ...
      sendText() {
        ...
        that.$request.doPost('/admin/sysMessage/subChat',{refId:that.refId,content:that.msgText,userId:that.$store.getters.userInfo.id}).then(res => {
          if (res.isOk) {
            that.getChatList();
            that.msgText=""
            setTimeout(() => {
