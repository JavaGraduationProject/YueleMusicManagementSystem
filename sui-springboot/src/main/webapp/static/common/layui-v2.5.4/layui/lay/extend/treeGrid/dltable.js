...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tools', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-edit', ELEM_HOVER = 'layui-table-hover'
        ...
        ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
            ...
            ,'{{# if(d.data.toolbar){ }}'
            ,'<div class="layui-table-tools"></div>'
            ,'{{# } }}'
...
//单元格编辑
        ...
        that.layBody.on('click', '*[lay-event]', function(){
            var othis = $(this)
                ...
                ,data = table.getDataList(that.key)[index];
            layui.event.call(this, MOD_NAME, 'tools('+ filter +')', {
    ...
    //自动完成渲染
    table.init();
    layui.link('/static/common/layui-v2.5.4/layui/lay/extend/treeGrid/treeGrid.css');//引入css
